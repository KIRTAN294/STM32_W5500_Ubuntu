...
void Input_Command(void)
{
    ...
    // printf("1 - Request_For_PDB_Data \n");
    printf("1 - Request_To_Kill\n");
    printf("2 - Request_To_OFF_Front\n");
    printf("3 - Request_To_OFF_Back\n");
    printf("4 - Request_For_Buzzer\n");
    ...
    if (fgets(input, sizeof(input), stdin) != NULL)
    {
        ...
        if ((is_digit && sscanf(input, "%d", (int *)&choice_enum) == 1 && choice_enum >= Request_To_Kill && choice_enum <= Request_For_Buzzer) != 1)
        {
...
void Command_To_STM(int TCP_client_fd, Commandtype choice_enum, uint8_t Numtimes)
{
    ...
    nRet = send(TCP_client_fd, &Packet, sizeof(Packet), 0);
